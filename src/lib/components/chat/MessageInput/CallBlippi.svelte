<script lang="ts">
	...
	import { synthesizeOpenAISpeech, synthesizeSoVITSSpeech, transcribeAudio } from '$lib/apis/audio';
	...
	const stopRecordingCallback = async (_continue = true) => {
		console.log("stopRecordingCallbackstopRecordingCallbackstopRecordingCallback _continue="+_continue)
		if ($showCallOverlay) {
	...
	const playAudio = (audio) => {
		console.log('playAudioplayAudioplayAudioplayAudioplayAudioplayAudioplayAudio');
		if ($showCallOverlay) {
			return new Promise((resolve) => {
				const audioElement = document.getElementById('audioElement') as HTMLAudioElement;

				console.log('document.getElementByIddocument.getElementByIddocument.getElementById audioElement:' + audioElement);
				if (audioElement) {
	...
	const fetchAudio = async (content) => {
		if (!audioCache.has(content)) {
			console.log("audioCache doest not has content:"+content);
			try {
				...
				if (res) {
					console.log('synthesizeSoVITSSpeech result content:' + content);
			...
			console.log("audioCache already has content:"+content);
		}
	...
	const monitorAndPlayAudio = async (id, signal) => {
		//一个消息一次？
		console.log("monitorAndPlayAudiomonitorAndPlayAudiomonitorAndPlayAudio messageId:"+id);
		let palying = false;
			...
			if (messages[id] && messages[id].length > 0) {
				...
				if (audioCache.has(content)) {
					...
					try {
						...
						if (!palying) {
							blippiVideoElement.currentTime = 0;  // 设置视频时间到开始位置
							blippiVideoElement.play();           // 开始播放
							palying = true;
						}
	...
	let blippiVideoElement; // 用于获取 Kid 1 的视频元素引用
</script>
...
<!--					<video-->
<!--						id="camera-feed"-->
<!--						bind:this={blippiVideoElement}-->
<!--						src="/static/blippi_animation_3.mov"-->
